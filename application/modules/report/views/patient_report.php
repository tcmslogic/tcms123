...
function getFilter(){
	...
	$.ajax({
		type:"POST",
		dataType: "json",
		url:"<?php echo site_url("report/getSearch");?>",
		cache:false,
...
<div id="breadcrumbs">
       <div class="patient_text"><h2>Patient Report</h2></div>
</div> 
<div id="search_box">
...
<div class="widget">
          ...
          <div class="widget_body"  id="holder">		
            ...
            <tr>
				...
				<th width="20%">Drop-outs</th>
                <th width="12%">No Follow-up</th>
			</tr>
			...
			if(count($patient_id)>0){$i=0; 
			 foreach($patient_id as $patient){ $id=$id.",".$patient->patient_id; $present="present"; $dropout="dropout"; $ns="noschedule";
			 $name = $this->mdl_report->getPatientName($patient->patient_id);
			 ?>           
			<tr >
				<td><?php echo ucfirst($name->given_name)." ".ucfirst($name->sur_name); ?></td>
				<td><?php echo $this->mdl_report->getCount(date('m'),date('Y'), $present, $patient->patient_id); ?></td>
                <td><?php echo $this->mdl_report->getCount(date('m'),date('Y'), $dropout, $patient->patient_id); ?></td>				
				<td><?php echo $this->mdl_report->getCount(date('m'),date('Y'), $ns, $patient->patient_id); ?></td>
			</tr>
			<?php }}else{?>
